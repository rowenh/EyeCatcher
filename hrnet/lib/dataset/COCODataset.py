...
class CustomImageDataset(Dataset):
    def __init__(self, root, dataset, data_format, transform=None,
        ...
        self.transform = transform
        self.target_transform = target_transform
        self.img_ids = [name.replace('.jpg','') for name in sorted(os.listdir(self.root+"/"+dataset),key=lambda x: int(x.split('_')[-1].replace('.jpg','')))]
    ...
    def _get_image_path(self, file_name):
        return os.path.join(self.root,self.dataset,file_name+'.jpg')
        # images_dir = os.path.join(self.root, 'images')
        # dataset = 'test2017' if 'test' in self.dataset else self.dataset
        # if self.data_format == 'zip':
        #     return os.path.join(images_dir, dataset) + '.zip@' + file_name
