...
def save_valid_image(image, joints, file_name, dataset='COCO'):
    ...
    for person in joints:
        ...
        python_list.append(person.tolist())

    python_obj = {'person':python_list}
...
def save_valid_video(images,joints_list,file_names,dataset='COCO',threshold=0.5,output="./output"):
#    fourcc = cv2.VideoWriter_fourcc(*"XVID")
 #   out = cv2.VideoWriter(output+'/output_body.avi', fourcc, 30.0, (images[0].shape[1],images[0].shape[0]))
    json_output= {}
    for i in range(len(joints_list)):
    #    image = cv2.cvtColor(images[i],cv2.COLOR_RGB2BGR)
     #   print(image.shape)
        python_list={}
        for j,person in enumerate(joints_list[i]):
            #color = np.random.randint(0, 255, size=3)
            #color = [int(i) for i in color]
 #           color = (255,0,0)
#            add_joints(image, person, color, dataset=dataset[i])
            python_list[str(j)]= person.tolist()
        
        python_obj = {'persons':python_list,'img':file_names[i]}
        json_output[str(i)] = python_obj
        #json_obj = json.dumps(python_obj)
        #with open(file_names[i].replace(".jpg",".json"),'w') as f:
        #    f.write(json_obj)
        #cv2.imwrite(file_names[i],image)
        #print(image.shape)
      #  out.write(image)
    with open(output+'/output_body.json','w') as f:
        f.write(json.dumps(json_output))
#    print("Created", output+"/output_body.avi")
    print("Created", output+"/output_body.json")
 #   out.release()
